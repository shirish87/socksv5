...
Server.prototype._onConnection = function(socket) {
  ...
  parser.on('error', function(err) {
    ...
    function accept(intercept, callback) {
      if (handled)
        return;
      handled = true;
      if (socket.writable) {
        if (intercept) {
          callback(socket, function (err, localAddress, localPort) {
            if (err) {
              // TODO: something better that maps err to SOCKS err
              socket.end(BUF_REP_DISALLOW);
              return;
            }

            var localbytes = ipbytes(localAddress || '127.0.0.1'),
